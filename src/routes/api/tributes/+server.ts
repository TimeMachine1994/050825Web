...
// GET /api/tributes - Get all tributes with optional filtering
export async function GET({ url, cookies }: RequestEvent) {
  try {
    ...
    const response = await tributeAPI.list(token || undefined, queryParams);
    
    return json(response);
  } catch (err: any) {
    console.error('Error fetching tributes:', err);
    throw error(err.status || 500, err.message || 'Failed to fetch tributes');
  }
}
...
export async function POST({ request, cookies }: RequestEvent) {
  try {
    ...
    // Call API to create tribute
    const response = await tributeAPI.create({ data: body }, token);
    
    return json(response);
  } catch (err: any) {
    console.error('Error creating tribute:', err);
    throw error(err.status || 500, err.message || 'Failed to create tribute');
  }
}